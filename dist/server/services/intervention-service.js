...
exports.getStudentAlerts = getStudentAlerts;
exports.getTeacherDashboard = getTeacherDashboard;
exports.getAnalytics = getAnalytics;
exports.triggerDetection = triggerDetection;
exports.resolveAlert = resolveAlert;
exports.getAlertDetails = getAlertDetails;
...
class InterventionService {
     ...
     */
    async checkTrigger(studentId, triggerType) {
        const now = Date.now();
        ...
        switch (triggerType) {
            ...
            case 'MASTERY_DROP': {
                ...
                const pastSnap = await this.db.collection('memory_records')
                    ...
                    .limit(1)
                    .get();
                if (latestSnap.empty || pastSnap.empty)
            ...
            case 'QUIZ_FAILURE_STREAK': {
                ...
                if (snap.size < 3)
                    return { triggered: false };
                const scores = snap.docs.map(doc => doc.data().score);
            ...
            case 'OVERDUE_ASSIGNMENT': {
                ...
                const snap = await this.db.collection('assignments')
                    .where('studentId', '==', studentId)
                    .where('dueDate', '<', firestore_1.Timestamp.fromDate(twoDaysAgo))
                    .get();
     ...
     */
    async logInterventionCreated(studentId, triggerType, severity) {
     ...
     * @private
     */
    async queueParentNotification(studentId, alertId, triggerType, context) {
     ...
     * @private
     */
    getSeverityForTrigger(triggerType) {
        switch (triggerType) {
 ...
 */
async function getStudentAlerts(studentId, userId, options = {}) {
 ...
 */
async function getTeacherDashboard(classId, userId) {
    const db = (0, firestore_1.getFirestore)();
    try {
 ...
 */
async function getAnalytics(classId, userId, options = {}) {
    const db = (0, firestore_1.getFirestore)();
    try {
        ...
        const snapshot = await db.collection('alerts')
            .where('classId', '==', classId)
            .where('createdAt', '>=', firestore_1.Timestamp.fromDate(dateThreshold))
            .get();
        return {
            period,
 ...
 */
async function triggerDetection(studentId) {
    try {
 ...
 * Resolve an alert
 */
async function resolveAlert(alertId, userId, data = {}) {
    const db = (0, firestore_1.getFirestore)();
    try {
 ...
 * Get alert details
 */
async function getAlertDetails(alertId, userId) {
    const db = (0, firestore_1.getFirestore)();
    try {
 ...
 */
function aggregateAlertsByTrigger(docs) {
    const result = {};
    docs.forEach(doc => {
        const data = doc.data();
