...
const trackingRateLimiter = (0, express_rate_limit_1.default)({
...
function prepareEventForStorage(event) {
    ...
    return {
        ...
        timestamp: eventTime,
        // TTL: 1 year from now
        expiresAt: firestore_1.Timestamp.fromMillis(Date.now() + 365 * 24 * 60 * 60 * 1000),
        storedAt: firestore_1.FieldValue.serverTimestamp()
    };
}
// --- Endpoints ---
/**
 ...
 */
router.post('/events', auth_1.verifyAuth, trackingRateLimiter, async (req, res) => {
    try {
        ...
        events.forEach((event) => {
            try {
                ...
                errors++;
                logger_1.logger.error('Error preparing event for batch', { error: err, event });
            }
        });
        // Async processing - commit batch without waiting for it to finish to return 202 quickly
        batch.commit().catch(err => {
            logger_1.logger.error('Firestore batch commit failed', { error: err });
        });
        ...
        logger_1.logger.error('Batch tracking error', { error });
 ...
 */
router.post('/event', auth_1.verifyAuth, trackingRateLimiter, async (req, res) => {
    try {
        ...
        logger_1.logger.error('Single tracking error', { error });
        return res.status(500).json({ error: 'Internal server error' });
    }
});
/**
 ...
 */
router.get('/student/:studentId/summary', auth_1.verifyAuth, async (req, res) => {
    try {
        const { studentId } = req.params;
        const days = parseInt(req.query.dateRange) || 7;
        ...
        logger_1.logger.error('Summary fetch error', { error });
        return res.status(500).json({ error: 'Internal server error' });
    }
});
/**
 ...
 */
router.get('/student/:studentId/timeline', auth_1.verifyAuth, async (req, res) => {
    try {
        const { studentId } = req.params;
        const limit = parseInt(req.query.limit) || 30;
        const offset = parseInt(req.query.offset) || 0;
        const actionTypes = req.query.actionTypes ? req.query.actionTypes.split(',') : null;
        ...
        logger_1.logger.error('Timeline fetch error', { error });
        return res.status(500).json({ error: 'Internal server error' });
    }
});
/**
 ...
 */
router.delete('/student/:studentId', auth_1.verifyAuth, async (req, res) => {
    try {
        const { studentId } = req.params;
        const { reason } = req.body;
        logger_1.logger.info(`Initiating data deletion for user ${studentId}`, { reason });
        const collectionRef = db.collection('tracking_events');
        ...
        logger_1.logger.error('Data deletion error', { error });
        return res.status(500).json({ error: 'Internal server error' });
    }
});
/**
 ...
 */
router.get('/analytics', auth_1.verifyAuth, auth_1.verifyAdmin, async (req, res) => {
    try {
        const days = parseInt(req.query.dateRange) || 30;
        const metric = req.query.metric; // e.g., 'views', 'clicks', 'completions'
        const startDate = Date.now() - days * 24 * 60 * 60 * 1000;
        if (!metric) {
        ...
        logger_1.logger.error('Analytics fetch error', { error });
        return res.status(500).json({ error: 'Internal server error' });
    }
});
